...
| `findActivity:saveOutput`              | `bool`                                                                                                        | `false`                               | Save output to `.biak/logs` txt file.                                                                                                                                                                                                 |
| `inspectCodeBaseline:target`           | Relative or absolute path to `.slnx`, `.sln`, or `.csproj`                                                    | Auto-discovery                        | Explicit target passed to `jb inspectcode`. If not set, biak tries to find exactly one `.slnx` / `.sln` / `.csproj` in the current directory.                                                                                         |
| `inspectCodeBaseline:path`             | Relative or absolute path to a file starting with `.editorconfig`                                             | Auto-discovery                        | Baseline file path for `inspectcode-baseline sync`. Precedence: CLI `--path` > this config value > auto-discovery by marker.                                                                                                          |
...
| `inspectCodeBaseline:debugMode`        | `bool`                                                                                                        | `false`                               | Print each attempted InspectCode candidate command (in launch order), print the generated SARIF path (when present), and keep the generated SARIF file in `.biak/logs/inspectcode-baseline` for `inspectcode-baseline init` / `sync`. |
...
* All `inspectCodeBaseline:...` settings are optional and are used by `dotnet biak inspectcode-baseline init` / `sync` ([docs](InspectCodeBaselineOverview)).
