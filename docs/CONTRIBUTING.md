# Contributing

### External contributor
* Open [issues](https://github.com/kurnakovv/biak/issues)
* Find or create an issue using [templates](https://github.com/kurnakovv/biak/issues/new/choose)
* Fork and clone repository
* Create branch based on dev branch (or main branch if critical bug), with the following format: `b-ISSUE_NUMBER`, for example `b-19`
* Make your changes
* Commit changes with issue number, with the following format: `YOUR_MESSAGE (#ISSUE_NUMBER)`,  for example `Some changes (#42)`. Commit message must contains present tense ([reason](https://stackoverflow.com/a/3580764/22817151)) and have a meaningful form ([click1](https://gist.github.com/robertpainsi/b632364184e70900af4ab688decf6f53), [click2](https://www.freecodecamp.org/news/how-to-write-better-git-commit-messages/))
* Create PR
* If there are any comments in the review, correct everything in your code and click like (👍) if the comment is resolved
* Done :)

Note: If you want to become a team member, then write to us, we will consider your application

### Team member
* Open [issues](https://github.com/kurnakovv/biak/issues)
* Select a task where you are assigned, if you don't have any tasks, then find or create an issue using [templates](https://github.com/kurnakovv/biak/issues/new/choose)
* Open kanban project ([click](https://github.com/users/kurnakovv/projects/17))
* Move task from `Backlog` --> `ToDo`
* Clone repository
* Create branch based on dev branch (or main branch if critical bug), with the following format: `b-ISSUE_NUMBER`, for example `b-19`
* Move task from `ToDo` --> `In progress`
* Make your changes
* Commit changes with issue number, with the following format: `YOUR_MESSAGE (#ISSUE_NUMBER)`,  for example `Some changes (#42)`. Commit message must contains present tense ([reason](https://stackoverflow.com/a/3580764/22817151)) and have a meaningful form ([click1](https://gist.github.com/robertpainsi/b632364184e70900af4ab688decf6f53), [click2](https://www.freecodecamp.org/news/how-to-write-better-git-commit-messages/))
* Create PR
* Move task from `In progress` --> `In review` and click `request review`
* If there are any comments in the review, correct everything in your code and click like (👍) if the comment is resolved
* Say to owner that task is done (he will merge your PR to production soon)
* Done :)
