...
public static class DisableCommand
{
    ...
    {
        return args is [CommandArgumentConstant.DISABLE];
    }
    ...
    public static async Task RunAsync(AppExecutionContext executionContext)
    {
        ...
        await executionContext.Out.WriteLineAsync(UIConstant.START_DISABLE);
        ...
        string content = await EditorconfigHelper.GetDisabledContentAsync(editorconfigMainContent, config, executionContext);
        await File.WriteAllTextAsync(editorconfigPaths.Value, content);

        await executionContext.Out.WriteLineAsync(UIConstant.END_DISABLE);
    }
}
