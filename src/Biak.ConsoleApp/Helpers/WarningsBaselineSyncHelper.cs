// Copyright (c) 2026 kurnakovv
// This file is licensed under the MIT License.
// See the LICENSE file in the project root for full license information.

using System.Text.RegularExpressions;

namespace Biak.ConsoleApp.Helpers;

/// <summary>
/// `dotnet biak warnings-baseline sync` command helper.
/// </summary>
public static class WarningsBaselineSyncHelper
{
    private const string BASELINE_DIAGNOSTIC_REGEXT = @"dotnet_diagnostic\.([A-Z][A-Z0-9]*)\.severity[^\r\n]*#\s*\^biak\^\s*baseline";

    // Matches a [{ ... }] section header that marks a baseline block.
    private static readonly Regex s_baselineSectionHeaderRegex = new(
        @"^\[\{[^\}]+\}\]\s*$",
        RegexOptions.Compiled
    );

    // Matches a dotnet_diagnostic baseline line and captures the diagnostic code.
    private static readonly Regex s_baselineDiagnosticRegex = new(@"^\s*" + BASELINE_DIAGNOSTIC_REGEXT, RegexOptions.Compiled);

    // Matches all diagnostic codes referenced in baseline entries anywhere in the content.
    private static readonly Regex s_allBaselineCodesRegex = new(BASELINE_DIAGNOSTIC_REGEXT, RegexOptions.Compiled);

    /// <summary>
    /// Returns <see langword="true"/> when <paramref name="content"/> contains at least one
    /// <c># ^biak^ baseline</c> entry.
    /// </summary>
    /// <param name="content">.editorconfig content.</param>
    /// <returns><see langword="true"/> if a baseline marker exists; otherwise <see langword="false"/>.</returns>
    public static bool HasBaselineMarker(string content)
    {
        return content.Contains("# ^biak^ baseline", StringComparison.Ordinal);
    }

    /// <summary>
    /// Returns <see langword="true"/> when <paramref name="filePath"/> resolves to a location
    /// inside <paramref name="baseDirectory"/> (directory-traversal protection).
    /// </summary>
    /// <param name="filePath">Path to validate.</param>
    /// <param name="baseDirectory">Base directory that the path must stay within.</param>
    /// <returns><see langword="true"/> when the path is safe; otherwise <see langword="false"/>.</returns>
    public static bool IsPathSafe(string filePath, string baseDirectory)
    {
        string fullBase = Path.GetFullPath(baseDirectory)
            .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)
            + Path.DirectorySeparatorChar;

        string fullFile = Path.GetFullPath(filePath, baseDirectory);

        return fullFile.StartsWith(fullBase, StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Returns all unique diagnostic codes that appear in baseline entries inside <paramref name="content"/>.
    /// </summary>
    /// <param name="content">.editorconfig content.</param>
    /// <returns>Unique diagnostic codes from baseline entries.</returns>
    public static HashSet<string> GetBaselineDiagnosticCodes(string content)
    {
        return s_allBaselineCodesRegex.Matches(content)
            .Select(m => m.Groups[1].Value)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Switches baseline severities between <c>suggestion</c> and <c>warning</c>
    /// for lines marked with <c># ^biak^ baseline</c>.
    /// </summary>
    /// <param name="content">.editorconfig content.</param>
    /// <param name="activate">
    /// <see langword="true"/> to replace <c>suggestion</c> with <c>warning</c>;
    /// <see langword="false"/> to replace <c>warning</c> with <c>suggestion</c>.
    /// </param>
    /// <returns>Updated content with baseline severities switched for the sync flow.</returns>
    public static string SetBaselineForBuild(string content, bool activate)
    {
        return activate
            ? content.Replace(
                "= suggestion # ^biak^ baseline",
                "= warning # ^biak^ baseline",
                StringComparison.Ordinal
            )
            : content.Replace(
                "= warning # ^biak^ baseline",
                "= suggestion # ^biak^ baseline",
                StringComparison.Ordinal
            );
    }

    /// <summary>
    /// Removes all baseline blocks (<c>[{&lt;files&gt;}]</c> header + diagnostic line + trailing blank lines)
    /// whose diagnostic code is NOT present in
    /// <paramref name="codesToKeep"/>.
    /// <para>
    /// A "baseline block" is a <c>[{...}]</c> section header immediately followed (ignoring blank lines) by a line that ends with <c># ^biak^ baseline</c>.
    /// Only blocks in the format generated by <c>warnings-baseline init</c> are matched;
    /// manually created multi-property sections are left untouched.
    /// </para>
    /// </summary>
    /// <param name="content">.editorconfig content.</param>
    /// <param name="codesToKeep">Diagnostic codes that should remain in the baseline.</param>
    /// <returns>Content with resolved baseline blocks removed.</returns>
    public static string RemoveBaselineFilters(string content, IReadOnlySet<string> codesToKeep)
    {
        string newline = content.Contains("\r\n", StringComparison.Ordinal) ? "\r\n" : "\n";
        string[] lines = content.Split(new string[] { newline }, StringSplitOptions.None);

        List<string> result = new(lines.Length);
        int i = 0;

        while (i < lines.Length)
        {
            string line = lines[i];

            if (s_baselineSectionHeaderRegex.IsMatch(line))
            {
                // Find the baseline diagnostic line (skip over leading blank lines between header and diagnostic)
                int diagIndex = i + 1;
                while (diagIndex < lines.Length && string.IsNullOrWhiteSpace(lines[diagIndex]))
                {
                    diagIndex++;
                }

                if (diagIndex < lines.Length)
                {
                    Match diagMatch = s_baselineDiagnosticRegex.Match(lines[diagIndex]);
                    if (diagMatch.Success)
                    {
                        string code = diagMatch.Groups[1].Value;

                        // Calculate the end of the block including trailing blank lines.
                        int blockEnd = diagIndex + 1;
                        while (blockEnd < lines.Length && string.IsNullOrWhiteSpace(lines[blockEnd]))
                        {
                            blockEnd++;
                        }

                        if (!codesToKeep.Contains(code))
                        {
                            // Drop the entire block (header + blank lines + diagnostic + trailing blanks).
                            i = blockEnd;
                            continue;
                        }
                    }
                }
            }

            result.Add(line);
            i++;
        }

        return string.Join(newline, result);
    }
}
