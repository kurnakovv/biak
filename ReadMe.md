...
* 🔎 Include/Execlude filter | Apply rules to all C# `[*.cs]` files except selected ones (e.g., `[{TestClass1.cs,TestClass2.cs}]`). While [!name] ([what?](https://editorconfig.org/#wildcards)) is supported, it affects all file types - not just C# files (soon).
